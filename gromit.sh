...
then
    ...
    echo -e "Solvent box\n    0" > $base-pbc.gro
    python3 -c "print(\"\".join([\"%10.5f\"%($PBCDIST*i) for i in [1,1,.7071068,0,0,0,0,.5,.5]]))" >> $base-pbc.gro
...
then
    ...
    PBCDIST=$(python3 -c "print(0.5 * $PBCDIST)")
...
then
    ...
    Q=$(python3 -c "print(int(0.5+$Salinity*($a+$b)*$NSOL/(55.4+$Salinity*($a+$b))))")
    ...
    m=${SaltCharge[0]}
    n=${SaltCharge[1]}
    U=$(python3 -c "print(int(0.5+( $Q*$n+$NCHARGE)/($n-$m)))")
    V=$(python3 -c "print(int(0.5+(-$Q*$m-$NCHARGE)/($n-$m)))")
    ...
    #NCL=$(python3 -c "print(max(min(0,$NCHARGE),int(0.5+0.5*($Salinity*$NSOL/(27.7+$Salinity)+$NCHARGE))))")
    #NNA=$((NCL-NCHARGE))
